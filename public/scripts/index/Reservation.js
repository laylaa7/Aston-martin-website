...
document.getElementById("firstNext").onclick = function () {
  ...
  if (!validModels.includes(selectedModel)) {
    document
      ...
      .setCustomValidity("Please select a valid car model.");
  ...
  if (cityFound && validModels.includes(selectedModel)) {
    ...
    if (!cityFound) {
      document
        ...
        .setCustomValidity("Please select a city from the list.");
...
document.getElementById("next-2").onclick = function () {
  ...
  if (
    ...
    !regex.test(telephoneInput.value.trim())
  ) {
  ...
  // Validation for contact method selection
  ...
  // If all validations passed, proceed to the next step
  if (valid) {
    event.preventDefault();
...
document.getElementById("submitBtn").onclick = function () {
  ...
  if (
    ...
    !cardHolderRegex.test(cardHolderInput.value.trim())
  ) {
